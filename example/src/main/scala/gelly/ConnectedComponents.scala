...
import gelly.examples.data.ConnectedComponentsDefaultData
 ...
 * [[gelly.examples.data.ConnectedComponentsDefaultData]]
 */
object ConnectedComponents {
  def main(args: Array[String]) {
    ...
    val components: org.apache.flink.api.java.DataSet[Vertex[Long, Long]] = graph.run(new GSAConnectedComponents[Long, Long, NullValue](maxIterations))

    // emit result
    if (fileOutput) {
      //      import scala.collection.JavaConversions._
      //      val ret = components.collect()//.map(f => f.f0 + "  " + f.f1).mkString("\n")
      //      Files.write(ret, new File(outputPath, "connected_components_result.csv"), Charsets.UTF_8)
      components.writeAsCsv(outputPath + "/result", "\n", ",", FileSystem.WriteMode.OVERWRITE)
  ...
  private final class InitVertices extends MapFunction[Long, Long] {
    override def map(id: Long): Long = id
  }
  ...
  private var edgesInputPath: String = _
